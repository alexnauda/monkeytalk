 ...
 */
Recorder
		.addEventHandler('type', 'change',
				function(event) {
					var tagName = event.target.tagName.toLowerCase();
					var type = event.target.type;
					if (('input' == tagName && ('text' == type
							|| 'password' == type || 'file' == type))
							|| 'textarea' == tagName) {
						this.record("type", this.findLocators(event.target),
								event.target.value);
					}
				});
	...
	if ('select' == tagName && event.target.multiple) {
		...
		for ( var i = 0; i < options.length; i++) {
			if (options[i]._wasSelected == null) {
				// is the focus was gained by mousedown event, _wasSelected
...
Recorder.addEventHandler('select', 'change', function(event) {
	...
	if ('textbox' == tagName) {
			log("url change: " + event.target.value);
			this.record("open", event.target, event.target.value);
	} else if ('select' == tagName) {
		if (!event.target.multiple) {
			var option = event.target.options[event.target.selectedIndex];
			this.log.debug('selectedIndex=' + event.target.selectedIndex);
			log("recording select");
//			this.record("select", this.findLocators(event.target), this
//					.getOptionLocator(option));
			this.record("select", event.target, option.value);
		} else {
			this.log.debug('change selection on select-multiple');
			
			// Updated record for MT
			var option = event.target.options[0];
			this.record("selectMultiple", event.target, option.value);
			
			/* Uncomment for normal selenium behavior
			var options = event.target.options;
			...
			for ( var i = 0; i < options.length; i++) {
				...
				if (options[i]._wasSelected != options[i].selected) {
					...
					if (options[i].selected) {
						this.record("addSelection", this
								.findLocators(event.target), value);
					} else {
						this.record("removeSelection", this
								.findLocators(event.target), value);
					}
					options[i]._wasSelected = options[i].selected;
				}
...
Recorder.addEventHandler('clickLocator', 'click', function(event) {
	if (event.button == 0) {
		...
		if (clickable) {
			...
			//
			// Recorder.addEventHandler('mouseoverLocator', 'mouseover',
			// function(event) {
...
Recorder.addEventHandler('rememberClickedElement', 'mousedown',
		function(event) {
