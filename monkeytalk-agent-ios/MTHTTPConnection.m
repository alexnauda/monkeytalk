...
@interface MTHTTPConnection (PrivateAPI)
...
@implementation MTHTTPConnection
...
/**
 * Sole Constructor.
 * Associates this new HTTP connection with the given MTAsyncSocket.
...
- (BOOL)isAuthenticated
{
	...
	MTHTTPAuthenticationRequest *auth = [[[MTHTTPAuthenticationRequest alloc] initWithRequest:request] autorelease];
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	{
		MTDDRange range1 = [[ranges objectAtIndex:i] ddrangeValue];
		
		NSUInteger j;
		for(j = i+1; j < [ranges count]; j++)
		{
			MTDDRange range2 = [[ranges objectAtIndex:j] ddrangeValue];
			
			MTDDRange iRange = MTDDIntersectionRange(range1, range2);
	...
	else
	{
		...
		else
		{
			...
			if([ranges count] == 1)
			{
				...
				MTDDRange range = [[ranges objectAtIndex:0] ddrangeValue];
...
- (CFHTTPMessageRef)prepareMultiRangeResponse:(UInt64)contentLength
{
	...
	{
		MTDDRange range = [[ranges objectAtIndex:i] ddrangeValue];
 ...
 * You may return any object that adopts the MTHTTPResponse protocol.
 * The MTHTTPServer comes with two such classes: MTHTTPFileResponse and MTHTTPDataResponse.
 * MTHTTPFileResponse is a wrapper for an NSFileHandle object, and is the preferred way to send a file response.
 * MTHTTPDataResponse is a wrapper for an NSData object, and may be used to send a custom response.
**/
- (NSObject<MTHTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	{
		return [[[MTHTTPFileResponse alloc] initWithFilePath:filePath] autorelease];
	}
 ...
 * The data parameter is the invalid HTTP header line, including CRLF, as read from MTAsyncSocket.
...
**/
- (BOOL)onSocketWillConnect:(MTAsyncSocket *)sock
{
 ...
 * It is called on the proper thread/runloop that MTHTTPServer configured our socket to run on.
**/
- (void)onSocket:(MTAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port
{
	...
	[asyncSocket readDataToData:[MTAsyncSocket CRLFData]
...
**/
- (void)onSocket:(MTAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
...
**/
- (void)onSocket:(MTAsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGES_RESPONSE_BODY)
	{
		...
		MTDDRange range = [[ranges objectAtIndex:rangeIndex] ddrangeValue];
...
**/
- (void)onSocket:(MTAsyncSocket *)sock willDisconnectWithError:(NSError *)err
{
//	NSLog(@"MTHTTPConnection: onSocket:willDisconnectWithError: %@", err);
}

/**
 * Sent after the socket has been disconnected.
**/
- (void)onSocketDidDisconnect:(MTAsyncSocket *)sock
{
	[self die];
}
