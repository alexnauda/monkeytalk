...
NSString *const MTAsyncSocketException = @"AsyncSocketException";
NSString *const MTAsyncSocketErrorDomain = @"AsyncSocketErrorDomain";

// This is a mutex lock used by all instances of MTAsyncSocket, to protect getaddrinfo.
...
@interface MTAsyncSocket (Private)
...
@implementation MTAsyncReadPacket
...
@interface MTAsyncWritePacket : NSObject
{
@public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation MTAsyncWritePacket
...
{
	if (theDelegate == NULL)
		[NSException raise:MTAsyncSocketException format:@"Attempting to accept without a delegate. Set a delegate first."];
	
	if (theSocket != NULL || theSocket6 != NULL)
		[NSException raise:MTAsyncSocketException format:@"Attempting to accept while connected or accepting connections. Disconnect first."];
...
- (BOOL)connectToHost:(NSString*)hostname onPort:(UInt16)port error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		...
		[NSException raise:MTAsyncSocketException format:message];
	}
...
{
	MTAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
	
	// Note: We use [self class] to support subclassing MTAsyncSocket.
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog (@"MTAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
		...
		(CFStreamClientContext *)(&theContext) ))
	{
		...
		NSLog (@"MTAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
		...
		(CFStreamClientContext *)(&theContext) ))
	{
		...
		NSLog (@"MTAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		NSLog (@"MTAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen (theWriteStream))
	{
		NSLog (@"MTAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the MTAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	NSError *err = nil;
	if ([self areStreamsConnected] && !(theFlags & kDidCallConnectDeleg))
	{
		...
		{
			NSLog (@"MTAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"AsyncSocketCFSocketError",
														 @"MTAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MTAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard MTAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:MTAsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:MTAsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard MTAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:MTAsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard MTAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:MTAsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	[ms appendString: [NSString stringWithFormat:@"<MTAsyncSocket %p", self]];
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	MTAsyncReadPacket *packet = [[MTAsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	{
		// Get new current read MTAsyncReadPacket.
		MTAsyncReadPacket *newPacket = [theReadQueue objectAtIndex:0];
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	MTAsyncWritePacket *packet = [[MTAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	{
		// Get new current write MTAsyncWritePacket.
		MTAsyncWritePacket *newPacket = [theWriteQueue objectAtIndex:0];
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog (@"MTAsyncSocket %p received unexpected CFSocketCallBackType %d.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"MTAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %d.", self, type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"MTAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %d.", self, type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	MTAsyncSocket *socket = [[(MTAsyncSocket *)pInfo retain] autorelease];
