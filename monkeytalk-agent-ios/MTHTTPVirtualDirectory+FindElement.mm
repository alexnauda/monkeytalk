//
//  MTHTTPVirtualDirectory+FindElement.h
//  iWebDriver
//
...
@implementation MTHTTPVirtualDirectory (FindElement)

// Converts an element |query| understood by the WebDriver wire protocol to a
// locator strategy supported by the browser automation MTatoms.
...
-(NSDictionary*) findElement:(NSDictionary*)query
  ...
  while (true) {
    id result = [self executeAtom:MTwebdriver::MTatoms::FIND_ELEMENT withArgs:args];
...
-(NSArray*) findElements:(NSDictionary*)query
  ...
  while (true) {
    NSArray* result =
        (NSArray*) [self executeAtom:MTwebdriver::MTatoms::FIND_ELEMENTS
                            withArgs:args];
    if ([result count] > 0) {
      return result;
    }
