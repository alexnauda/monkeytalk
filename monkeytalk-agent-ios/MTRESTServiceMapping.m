...
@implementation MTRESTServiceMapping
...
- (id)init {
  ...
  serverRoot_ = [[MTHTTPVirtualDirectory alloc] init];

  // This makes up for a bug in the java http client (r733). We forward
  // requests for /session to /hub/session.
  [serverRoot_ setResource:[MTHTTPStaticResource redirectWithURL:@"/hub/session/"]
                  withName:@"session"];

  // The root of our REST service.
  MTHTTPVirtualDirectory *restRoot = [[[MTHTTPVirtualDirectory alloc] init] autorelease];
  [serverRoot_ setResource:restRoot withName:@"hub"];
  
  MTHTTPDataResponse *response =
    [[MTHTTPDataResponse alloc]
                 initWithData:[@"<html><body><h1>iWebDriver ready.</h1></body>"
                               "</html>"
            dataUsingEncoding:NSASCIIStringEncoding]];
  
  [restRoot setIndex:[MTHTTPStaticResource resourceWithResponse:response]];
  [response release];
  
  [restRoot setResource:[[[MTSessionRoot alloc] init] autorelease]
               withName:@"session"];
...
- (NSObject<MTHTTPResponse> *)httpResponseForRequest:(CFHTTPMessageRef)request {
  ...
  if ([response isKindOfClass:[MTHTTPRedirectResponse class]]) {
    NSURL *uri = [(NSURL *)CFHTTPMessageCopyRequestURL(request) autorelease];
    [(MTHTTPRedirectResponse *)response expandRelativeUrlWithBase:uri];
  }
