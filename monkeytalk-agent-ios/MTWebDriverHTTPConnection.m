...
@implementation MTWebDriverHTTPConnection

- (NSObject<MTHTTPResponse> *)httpResponseForRequest:(CFHTTPMessageRef)theRequest {
  // Forward the message to our |MTRESTServiceMapping| instance.
  return [[server delegate] httpResponseForRequest:theRequest];
}

- (NSObject<MTHTTPResponse> *)httpResponseForMethod:(NSString *)method
                                              URI:(NSString *)path {
  return [self httpResponseForRequest:request];
}

- (BOOL)supportsMethod:(NSString *)method atPath:(NSString *)path {
  // It is up to the |MTHTTPResource| to determine if it supports a given method.
...
- (NSData *)preprocessErrorResponse:(CFHTTPMessageRef)response {
  ...
  if(CFHTTPMessageGetResponseStatusCode(response) == 404) {
      NSString *msg = [MTBuildStamp serverTestPage];
