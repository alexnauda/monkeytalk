...
@implementation MTWebDriverRequestFetcher

@synthesize serviceMapping = serviceMapping_;
@synthesize viewController = viewController_;
@synthesize status = status_;

static MTWebDriverRequestFetcher *singleton = nil;
+ (MTWebDriverRequestFetcher*) sharedInstance {
  if (singleton == nil) {
    singleton = [[MTWebDriverRequestFetcher alloc] init];
  }	
  return singleton;
}
...
+ (NSDictionary*) sendRequest:(NSURLRequest*) request {
  ...
  return [MTWebDriverRequestFetcher convertJsonDataToDictionary:fetchedData];  
}
 
// Get response data from a response.
+ (NSData*) getResponseData:(id<MTHTTPResponse,NSObject>) response {
...
-(void)parseWebDriverRequest:(NSDictionary*)wdRequest
  ...
  *actionsRef = nil;
  ...
  if ([MTWebDriverRequestFetcher isFindElementQuery:(*queryRef)]) {
...
-(id<MTHTTPResponse,NSObject>)httpResponseForQuery:(NSString *)query
  ...
  id<MTHTTPResponse,NSObject> response = [serviceMapping_ httpResponseForRequest:message];
...
-(id<MTHTTPResponse,NSObject>) httpResponseForActions:(NSArray*) actions
                                          onElement:(NSData*) findElementResp{
  NSDictionary* elemResp = [MTWebDriverRequestFetcher convertJsonDataToDictionary:findElementResp];
  ...
  while (action = (NSArray*)[enumerator nextObject]) {
    ...
    NSData *data = [MTWebDriverRequestFetcher convertJsonArrayToData:actionDataItems];
...
+(NSDictionary*) sendResponse:(id<MTHTTPResponse,NSObject>) response
  ...
  } else if ([response isKindOfClass:[MTHTTPRedirectResponse class]]) {
    // Note header location should be the part relative to connectorAddr. Here we return path.
    cmdRespStatus = @"302";
    [cmdRespHeaders setValue:@"text/plain" forKey:@"Content-Type"];
    NSString* redirectedDest = [(MTHTTPRedirectResponse* )response destination];
  ...
  NSData *responseData = [MTWebDriverRequestFetcher convertDictionaryToData:responseBodyDict];
  ...
  return [MTWebDriverRequestFetcher sendRequest:requestPostResp];
}
...
- (BOOL) fetchRequestFrom:(NSURL*) urlFetchRequest sendResponseTo:(NSURL*) urlSendResponse{
  ...
  NSDictionary* wdRequest = [MTWebDriverRequestFetcher sendRequest:fetchRequest];
  ...
  while (wdRequest != nil) {
    ...
    id<MTHTTPResponse,NSObject> response = [self httpResponseForQuery:query method:method data:data];
    NSData *responseData = [MTWebDriverRequestFetcher getResponseData:response];
    
    if ([MTWebDriverRequestFetcher isFindElementQuery:query] && actions != nil && response != nil) {
      id<MTHTTPResponse,NSObject> actionResponse = 
          [self httpResponseForActions:actions onElement:responseData];
      if (actionResponse !=nil) {
        response = actionResponse;
        responseData = [MTWebDriverRequestFetcher getResponseData:actionResponse];
      }
    }
    
    // Pack some useful information into the response body.
    NSString *respStr = [MTWebDriverRequestFetcher generateResponseBodyString:responseData  
                                                          withReqArriveTime:reqArriveTime
                                                       withRespSendingSTime:[NSDate date]];
    
    wdRequest = [MTWebDriverRequestFetcher sendResponse:response 
