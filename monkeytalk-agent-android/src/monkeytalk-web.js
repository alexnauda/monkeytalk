if (MonkeyTalk == undefined || adapters == undefined) {
	...
	MonkeyTalk.monkeyElement = function(element) {
		...
		monkeyElement["tagName"] = element.tagName != undefined ? element.tagName
				: null;
		monkeyElement["className"] = element.className != undefined ? element.className
				: null;
		monkeyElement["value"] = element.value != undefined ? element.value
				: null;
		monkeyElement["textContent"] = element.textContent != undefined ? element.textContent
				: null;
		...
		monkeyElement["title"] = element.title != undefined ? element.title
				: null;
		monkeyElement["component"] = mtc.componentType != undefined ? mtc.componentType
				: "View";
		monkeyElement["ordinal"] = ordinal != undefined ? ordinal
				: null;
		monkeyElement["checked"] = element.checked != undefined && element.checked == true ? "true"
				: "false";
				
		if (element.type != undefined && element.type == "radio") {
			if (element.name != undefined) {
				monkeyElement["selected"] = document.querySelector('input[name="' + element.name +'"]:checked').value;
			}
		}
	...
	MonkeyTalk.record = function(event, action) {
		...
		if (MonkeyTalk.isIos() == true) {
			...
			xPathResult = 'ComponentType*' + mtc.componentType + ';MonkeyId*'
					+ mtc.monkeyId + ';Action*' + mtaction + ';Args*'
					+ mtc.args;
	...
	MonkeyTalk.recordTap = function(x, y) {
		...
		if (mtaction == null || mtaction == 'change')
			mtaction = mtc.action;
		...
		console.log('component: ' + mtc.componentType + ' monkeyId: '
				+ mtc.monkeyId + ' action: ' + mtaction + ' args: ' + mtc.args);
		// console.log ('{' + ''component':' + ''' + mtc.componentType + ''' +
		// ',' + ''monkeyId':' + ''' + mtc.monkeyId + ''' + ',' + ''action':' +
		// ''' + mtaction + ''' + ',' + ''args':' + ''' + mtc.args + ''' + '}');
	};
	...
	adapters.tag.prototype.mid = function(e, findOrdinal) {
		...
		if (e.type == undefined) {
			monkeyId = e.getAttribute('id') || e.getAttribute('name')
					|| e.getAttribute('value') || e.getAttribute('title')
					|| e.getAttribute('class') || e.getAttribute('alt')
					|| e.textContent || e.getAttribute('src')
					|| e.getAttribute('href');
			...
			if (e.type.toLowerCase() != 'submit'
				...
				monkeyId = e.id || e.name || e.title || e.getAttribute('class')
						|| e.styleClass;
			else {
				monkeyId = e.id || e.name || e.value || e.textContent
						|| e.title || e.getAttribute('class') || e.styleClass;
			}
		} else {
			monkeyId = e.getAttribute('id') || e.getAttribute('name')
					|| e.getAttribute('value') || e.getAttribute('title')
		...
		if (monkeyId == e.textContent) {
			...
			isTextContentLegal = isTextContentLegal
					|| monkeyId.substring(monkeyId.length - 1, monkeyId.length) == ' ';
			isTextContentLegal = isTextContentLegal
					|| (this.element.textContent == monkeyId && this.element.innerHTML
							.indexOf('<') !== -1);
		...
		if (findOrdinal && this.componentType != undefined
				&& this.componentType.toLowerCase() != 'radiobuttons')
	...
	adapters.tag.prototype.ordinalMid = function(e, monkeyId) {
		...
		for ( var i = 0; i < elements.length; i++) {
			...
			if (monkeyId == elementMid) {
				// console.log('elementMid: ' + e.textContent + ' current: ' +
				// element.textContent + ' ordinal: ' + i);
				if (element == e) {
	...
	adapters.tag.prototype.ordinal = function(e) {
		...
		for ( var i = 0; i < elements.length; i++) {
			...
			var isTagMatch = (element.tagName.toLowerCase() == e.tagName
					.toLowerCase());
			...
			if (element == e) {
				...
				if (isInput && !isElementRadio && !isElementCheckBox) {
					ordinal++;
					// log('ordinal: ' + element.tagName + ' type: ' +
					// element.type + ' ordinal: ' + ordinal + ' element: ' +
					// e.type);
	...
	adapters.tag.prototype.getMTCommand = function(element, action) {
		...
		shouldFindOrdinal = shouldFindOrdinal
				|| monkeyId.substring(0, 1) == ' ';
		shouldFindOrdinal = shouldFindOrdinal
				|| monkeyId.substring(monkeyId.length - 1, monkeyId.length) == ' ';
		shouldFindOrdinal = shouldFindOrdinal
				|| (element.textContent == monkeyId && element.innerHTML
						.indexOf('<') !== -1);
	...
	adapters.input = function(element) {
		...
		this.componentType = this.componentNames[this.element.type
				.toLowerCase()];
	};
	...
	adapters.input.prototype.getMTCommand = function(element, action) {
		var mtcommand = adapters.tag.prototype.getMTCommand.call(this, element,
				action);
		if (element.type.toLowerCase() == 'radio') {
			...
			mtcommand.monkeyId = adapters.tag.prototype.mid.call(this, element,
					true);
	...
	adapters.table.prototype.getMTCommand = function(command, target, value) {
		var mtcommand = adapters.tag.prototype.getMTCommand.call(this, command,
				target, value);
		...
		if (!mtcommand.monkeyId)
			mtcommand.monkeyId = adapters.tag.prototype.ordinal.call(this,
					table);

		if (cell && cell.textContent.length > 0) {
			console.log('set action select');
			mtcommand.action = 'select';
			mtcommand.args = cell.textContent;
		} else {
	...
	for ( var i = 0; i < mtframes.length; i++) {
		...
		// console.log('element: ' + mtframe.tagName + ':' +
		// mtframe.getAttribute('id'));
